...
const BillContent = () => {
  ...
  const stats = useMemo(() => {
    ...
    const yearMonth = formatMonthKey(startDate);
    const chartData = [];
  ...
  const getDateLabel = (dateStr: string) => {
    const dayBeforeYesterday = dayjs().subtract(2, 'day').format(DATE_FORMAT.DATE_KEY);
    ...
    return `${formatDisplayMonthDay(date)} ${weekday}`;
  };
